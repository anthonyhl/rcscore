...
import com.orangelabs.rcs.core.security.cert.X509KeyManagerWrapper;
...
public class AndroidSecureSocketConnection extends AndroidSocketConnection {
	 ...
	 * Constructor
	 */
	public AndroidSecureSocketConnection(KeyStoreManager keyStore) {
		super();
		keys = keyStore;
	}
	 ...
	 * @param fingerprint 
	 */
	public AndroidSecureSocketConnection(String fingerprint, KeyStoreManager keyStore) {
		super(null);
		keys = keyStore;
	 ...
	 */
	public AndroidSecureSocketConnection(SSLSocket socket, KeyStoreManager keyStore) {
		super(socket);
		keys = keyStore;
	}
	...
	public String getFingerprint(String algorithm, SSLSocket socket) {
		...
		try{
			if ((socket != null) && (socket.getSession() != null)) { 
				...
				if (certs.length > 0) {
					result = keys.getCertFingerprint(certs[0], algorithm);
				}
			} else {
				if (logger.isActivated()) {
					if (socket == null) {
	...
	private synchronized SSLSocketFactory getSslFactory() throws IOException {
		...
		if (mSslSocketFactory == null) {
			try {
				...
				String keyStoreType = keys.getKeystoreType();
				String keyStoreFile = keys.getKeystorePath();
				String trustStoreFile = keys.getKeystorePath();
				char[] keyStorePassword = keys.getKeystorePassword().toCharArray();
