...
public class ChatServiceImpl extends IChatService.Stub {
	...
	public void notifyRegistrationEvent(boolean state) {
		...
		synchronized (lock) {
            ...
            // TODO tct-stack, handle send queued chats caused by registration status
			// If not, these message will only sent when it's session established by other operations
            if (state) {
//                handleSendAllQueuedChatMessages();
            }
		}
	}
    ...
    public IGroupChat initiateGroupChat(List<ContactId> contacts, String subject) throws ServerApiException {
		...
		try {
			...
			mMessagingLog.addGroupChatEvent(session.getContributionID(), session.getRemoteContact(), GroupChat.State.INITIATING);
			...
			String contributionId = mCore.getSipManager().getContributionId();
			mMessagingLog.addGroupChat(
					contributionId, null, subject, participants,
    ...
    /**********************************************************************************
     *  tct-stack add for CMCC message modes: 1) pager mode; 2) large mode in OTO & OTM
     **********************************************************************************/
     ...
     * @throws ServerApiException
     */
    public IOneToOneChat getOneToOneChatExt(List<ContactId> contacts) throws ServerApiException {
    ...
    /**
     * Receive a new pager mode chat invitation
     *
     * @param session Chat session
     */
    public void receiveOneOneChatPagerModeInvitation(PagerModeSession session) {
        ContactId contact = session.getRemoteContact();
        if (logger.isActivated()) {
            logger.info("Receive chat pager mode invitation from " + contact + " (display="
                    + session.getRemoteDisplayName() + ")");
        }
        // Update displayName of remote contact
        mContactsManager.setContactDisplayName(contact, session.getRemoteDisplayName());
        ...
        if (oneToOneChat == null) {
            oneToOneChat = new OneToOneChatImpl(contact, mOneToOneChatEventBroadcaster,
                    mImService, mMessagingLog, mRcsSettings, this);
        }
        session.addListener(oneToOneChat.new ChatMessageListener(session.getFirstMessage()));
    }

    /**
     * Receive a new large mode chat invitation
     *
     * @param session Chat session
     */
    public void receiveOneOneChatLargeModeInvitation(OneToOneChatSession session) {
        ContactId contact = session.getRemoteContact();
        if (logger.isActivated()) {
            logger.info("Receive chat large mode invitation from " + contact + " (display="
                    + session.getRemoteDisplayName() + ")");
        }
        // Update displayName of remote contact
        mContactsManager.setContactDisplayName(contact, session.getRemoteDisplayName());
