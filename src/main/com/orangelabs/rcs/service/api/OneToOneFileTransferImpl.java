...
public class OneToOneFileTransferImpl extends IFileTransfer.Stub implements FileSharingSessionListener {
	...
	public void handleSessionInvited() {
		...
		synchronized (lock) {
            // @tct-stack add begin {
            if (session.isLargeMode()) {
                mPersistentStorage.addFileTransferExt(session.getConversationID(), getRemoteContact(),
                        Direction.INCOMING, session.getContent(), session.getFileicon(), FileTransfer.State.INVITED,
                        ReasonCode.UNSPECIFIED);
            } else
            // } end
