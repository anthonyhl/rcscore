...
import com.orangelabs.rcs.core.richcall.geoloc.OriginatingGeolocTransferSession;
import com.orangelabs.rcs.core.richcall.geoloc.TerminatingGeolocTransferSession;
import com.orangelabs.rcs.core.richcall.image.ImageTransferSession;
import com.orangelabs.rcs.core.richcall.image.OriginatingImageTransferSession;
import com.orangelabs.rcs.core.richcall.image.TerminatingImageTransferSession;
import com.orangelabs.rcs.core.richcall.video.OriginatingVideoStreamingSession;
import com.orangelabs.rcs.core.richcall.video.TerminatingVideoStreamingSession;
...
public class RichcallService extends ImsService {
     ...
     * @return Boolean
     */
	private boolean isCallConnectedWith(ContactId contact) {		
		
		return getImsModule().isCallConnectedWith(contact);
	}
	...
	@Override
	public boolean handleInvite(SipRequest request) {
		...
		if (isTagPresent(sdp, "msrp") &&
    			request.isFeatureTagPresent(FeatureTags.FEATURE_3GPP_VIDEO_SHARE) &&
    				(request.isFeatureTagPresent(FeatureTags.FEATURE_3GPP_IMAGE_SHARE) ||
    						request.isFeatureTagPresent(FeatureTags.FEATURE_3GPP_IMAGE_SHARE_RCS2))) {
    		// Image sharing
    		if (RcsSettings.getInstance().isImageSharingSupported()) {
	    		if (logger.isActivated()) {
	    			logger.debug("Image content sharing transfer invitation");
	    		}
    			receiveImageSharingInvitation(request);
    		} else {
    		...
    		if (RcsSettings.getInstance().isVideoSharingSupported()) {
    			...
    			receiveVideoSharingInvitation(request);
    		} else {
				// Service not supported: reject the invitation with a 603 Decline
				if (logger.isActivated()) {
    		...
    		return true;
	    		...
	    		request.isFeatureTagPresent(FeatureTags.FEATURE_3GPP_VIDEO_SHARE) &&
	    			request.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_GEOLOCATION_PUSH)) {
    		// Geoloc sharing
    		if (RcsSettings.getInstance().isGeoLocationPushSupported()) {
	    		if (logger.isActivated()) {
	    			logger.debug("Geoloc content sharing transfer invitation");
	    		}
    			receiveGeolocSharingInvitation(request);
    		} else {
				// Service not supported: reject the invitation with a 603 Decline
				if (logger.isActivated()) {
	...
	public void receiveImageSharingInvitation(SipRequest invite) {
		...
		try {
			contact = ContactUtils.createContactId(invite.getAssertedIdentity());
		...
		getImsModule().getListener().handleContentSharingTransferInvitation(session);
	...
	public void receiveVideoSharingInvitation(SipRequest invite) {
		...
		getImsModule().getListener().handleContentSharingStreamingInvitation(session);
