...
import com.orangelabs.rcs.protocol.rtp.RtpUtils;
...
public class OriginatingVideoStreamingSession extends VideoStreamingSession {
    ...
    public void run() {
        try {
            ...
            //buildVideoSdp(ipAddress, getVideoPlayer().getLocalRtpPort(), getVideoPlayer().getSupportedCodecs());
            ...
            SipRequest invite = getDialogPath().createInvite(featureTags, featureTags, sdp);
	...
	String buildVideoSdp(String ipAddress, int localRtpPort, VideoCodec[] supportedCodecs) {
		SimpleSessionDescription o = new SimpleSessionDescription(System.currentTimeMillis(), ipAddress);
		Media m = o.newMedia("video", localRtpPort, 1, "RTP/AVP");
		int framerate = 0;
		for (VideoCodec v : supportedCodecs) {
			framerate = Math.max(framerate, v.getFrameRate());
			m.setRtpPayload(v.getPayloadType(), v.getEncoding(), v.getParameters(),
					v.getVideoWidth(), v.getVideoHeight());
		}
		m.setAttribute("framerate", String.valueOf(framerate));
		m.setExtMap(RtpUtils.RTP_DEFAULT_EXTENSION_ID, SdpOrientationExtension.VIDEO_ORIENTATION_URI);

		o.setAttribute(SdpUtils.DIRECTION_SENDONLY, "");
		return o.encode();
	}
