...
public class IPCallService extends ImsService {
    ...
    protected boolean handleInvite(SipRequest request) {
    	if (request.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_IP_VOICE_CALL) &&
    			request.isFeatureTagPresent(FeatureTags.FEATURE_3GPP_IP_VOICE_CALL))	{
    		// IP voice call
    		if (RcsSettings.getInstance().isIPVoiceCallSupported()) {
	    		if (logger.isActivated()) {
	    			logger.debug("IP Voice call invitation");
	    		}
    			receiveIPCallInvitation(request, true, false);
    		} else {
    	...
    	if (request.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_IP_VOICE_CALL) &&
    			request.isFeatureTagPresent(FeatureTags.FEATURE_3GPP_IP_VOICE_CALL) &&
    				request.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_IP_VIDEO_CALL))	{
    		// IP video call
    		if (RcsSettings.getInstance().isIPVideoCallSupported()) {
    			if (logger.isActivated()) {
    				logger.debug("IP video call invitation");
    			}
    			receiveIPCallInvitation(request, true, true);
    		} else {
    			// Service not supported: reject the invitation with a 603 Decline
    			if (logger.isActivated()) {
	...
	public void receiveIPCallInvitation(SipRequest invite, boolean audio, boolean video) {
		...
		getImsModule().getListener().handleIPCallInvitation(session);
