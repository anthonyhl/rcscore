...
public class SessionTimerManager extends PeriodicRefresher {
    ...
    private void sessionRefreshForUAC() {
        try {
            ...
            SipRequest reInvite = session.getDialogPath().createReInvite();
    ...
    private void sendReInvite(SipRequest reInvite) throws SipException, CoreException {
        ...
        if (ctx.isSipResponse()) {
            ...
            if (ctx.getStatusCode() == 200) {
				...
				SipManager r = session.getImsService().getImsModule().getSipManager();
                session.getDialogPath().sendSipAck();
            ...
            if (ctx.getStatusCode() == 407) {
                ...
                SipRequest newReInvite = session.getDialogPath().createReInvite();
    ...
    public void receiveReInvite(SipRequest reInvite) {
        try {
			...
			ContactHeader contact = session.getDialogPath().getSipStack().getContact();
            SipResponse resp = reInvite.create200OkReInviteResponse(contact);

            // The signalisation is established
            session.getDialogPath().sigEstablished();

            // Send response
            SipTransactionContext ctx = session.getSipManager().sendSipMessageAndWait(resp);
	...
	public void receiveUpdate(SipRequest update) {
		try {
	        ...
	        SipResponse resp = SipUtils.create200OkUpdateResponse(session.getDialogPath(), update);
