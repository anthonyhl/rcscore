...
public class TerminatingHttpFileSharingSession extends HttpFileTransferSession implements HttpTransferEventListener {
	...
	public TerminatingHttpFileSharingSession(ImsService parent, ChatSession chatSession,
		...
		super(parent,
				ContentManager.createMmContent(
						ContentManager.generateUriForReceivedContent(
								fileTransferInfo.getFilename(),
								fileTransferInfo.getFileType()),
								fileTransferInfo.getFileSize(), 
								fileTransferInfo.getFilename()),
				contact, 
				PhoneUtils.formatContactIdToUri(contact, parent.getUser().getHomeDomain()),
	...
	public TerminatingHttpFileSharingSession(ImsService parent, MmContent content, FtHttpResumeDownload resume) {
		super(parent, content, resume.getContact(), 
				PhoneUtils.formatContactIdToUri(resume.getContact(), parent.getUser().getHomeDomain()),
	...
	protected void sendDeliveryReport(String status) {
		...
		InstantMessagingService imService = ImsModule.getInstance().getInstantMessagingService();
		if (mGroupFileTransfer) {
