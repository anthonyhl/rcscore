...
import com.orangelabs.rcs.core.im.chat.cpim.CpimIdentity;
import com.orangelabs.rcs.core.im.chat.cpim.CpimMessage;
import com.orangelabs.rcs.core.im.chat.cpim.CpimParser;
import com.orangelabs.rcs.core.im.chat.event.ConferenceEventSubscribeManager;
...
public abstract class GroupChatSession extends ChatSession {
	...
	public void sendChatMessage(ChatMessage msg) {
		String from = getImsService().getUser().getPublicAddress();
		...
		String homeDomain = getImsService().getUser().getHomeDomain();
		if (useImdn) {
			networkContent =
					ChatUtils.buildCpimMessageWithImdn(from, to, msgId,
					msg.getContent(), mimeType, homeDomain);
	...
	public void sendIsComposingStatus(boolean status) {
		String from = getImsService().getUser().getPublicUri();
		String to = ChatUtils.ANOMYNOUS_URI;
		String msgId = IdGenerator.generateMessageID();
		String content = ChatUtils.buildCpimMessage(from, to, 
				IsComposingInfo.buildIsComposingInfo(status),
    ...
    public void sendMsrpMessageDeliveryStatus(String fromUri, String toUri, String msgId, String status) {
		...
		String content = ChatUtils.buildCpimDeliveryReport(getImsService().getUser().getPublicUri(), 
				toUri, imdn, 
				getImsService().getUser().getHomeDomain());
	...
	public void addParticipant(ContactId participant) {
		try {
    		...
    		String contactUri = PhoneUtils.formatContactIdToUri(participant, getImsService().getUser().getHomeDomain());
	        SipRequest refer = getDialogPath().createRefer(contactUri, getSubject(), getContributionID());
    		SipTransactionContext ctx = getImsService().getSipManager().sendSubsequentRequest(getDialogPath(), refer);
            ...
            if (ctx.getStatusCode() == 407) {
    	        ...
    	        refer = getDialogPath().createRefer(contactUri, getSubject(), getContributionID());
	...
	public void addParticipants(Set<ContactId> participants) {
		try {
			...
			String resourceList = ChatUtils.generateChatResourceList(participants, getImsService().getUser().getHomeDomain());
	        SipRequest refer = getDialogPath().createReferResource(resourceList, getSubject(), getContributionID());
	        SipTransactionContext ctx = getImsService().getSipManager().sendSubsequentRequest(getDialogPath(), refer);
            ...
            if (ctx.getStatusCode() == 407) {
				...
				// Generate the resource list for given participants
    	        refer = getDialogPath().createReferResource(resourceList, getSubject(), getContributionID());
    ...
    /* (non-Javadoc)
     * @see com.orangelabs.rcs.core.service.im.chat.ChatSession#msrpDataReceived(java.lang.String, byte[], java.lang.String)
     */
    @Override
			...
			if (ChatUtils.isTextPlainType(contentType)) {
				...
				if (ChatUtils.isApplicationIsComposingType(contentType)) {
					...
					if (ChatUtils.isMessageImdnType(contentType)) {
						// Delivery report
						try {
							ContactId me = ContactUtils.createContactId(getImsService().getUser().getPublicUri());
