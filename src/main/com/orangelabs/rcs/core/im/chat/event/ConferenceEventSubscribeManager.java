...
package com.orangelabs.rcs.core.im.chat.event;
...
public class ConferenceEventSubscribeManager extends PeriodicRefresher {
			...
			try {
				...
				if (conference != null) {
					...
					for (User user : users) {
						...
						if (user.isMe() || contact.equals(imsModule.getUser().getUsername())) {
							// By-pass me
							continue;
						}
    ...
    public void terminate() {
		...
		if ((imsModule != null) &&
				imsModule.isRegistered() && subscribed) {
				unSubscribe();
    	}
			...
			public void run() {
				...
				try {
					...
					if (dialogPath == null) {
						...
						dialogPath = imsModule.getSipManager().createServiceDialogPath(getPresentity());
					} else {
	...
	private void handle200OK(SipTransactionContext ctx) {
        ...
        imsModule.getInstantMessagingService().addGroupChatConferenceSubscriber(dialogPath.getCallId(), session);
	}
