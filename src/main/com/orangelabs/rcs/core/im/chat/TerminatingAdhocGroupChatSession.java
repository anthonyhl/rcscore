...
public class TerminatingAdhocGroupChatSession extends GroupChatSession implements MsrpEventListener {
	...
	public TerminatingAdhocGroupChatSession(ImsService parent, SipRequest invite,
		...
		setRemoteDisplayName(SipUtils.getDisplayNameFromUri(invite.getAssertedIdentity()));
		
		// Set contribution ID
		String chatId = invite.getContributionId();
		setContributionID(chatId);
		
		setConversationID(invite.getConversationId());
	...
	public void run() {
		...
		try {
            ...
            SipResponse resp = getDialogPath().create200OkInviteResponse(getFeatureTags(), getAcceptContactTags(), sdp	);
