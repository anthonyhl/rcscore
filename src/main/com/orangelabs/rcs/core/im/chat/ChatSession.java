...
import com.orangelabs.rcs.core.im.chat.iscomposing.IsComposingManager;
import com.orangelabs.rcs.core.im.chat.standfw.TerminatingStoreAndForwardMsgSession;
import com.orangelabs.rcs.core.im.chat.standfw.TerminatingStoreAndForwardNotifSession;
...
public abstract class ChatSession extends ImsServiceSession implements MsrpEventListener {
	...
	public ChatSession(ImsService parent, ContactId contact, String remoteUri, Set<ParticipantInfo> participants, RcsSettings rcsSettings) {
		...
		mMsrpMgr = parent.createMsrpManager();
	}
			...
			try {
				...
				if (cpimMsg != null) {
                    ...
                    if (isFToHTTP) {
                        ...
                        // @tct-stack add begin {
                        else if (ChatUtils.isMcloudType(contentType)) {
                            ChatMessage msg = new ChatMessage(cpimMsgId, contact,
                                    cpimMsg.getMessageContent(), McloudDocument.MIME_TYPE, date, null);
                            receive(msg, imdnDisplayedRequested);
                        } else if (ChatUtils.isVemoticonType(contentType)) {
                            ChatMessage msg = new ChatMessage(cpimMsgId, contact,
                                    cpimMsg.getMessageContent(), VemoticonDocument.MIME_TYPE, date, null);
                            receive(msg, imdnDisplayedRequested);
                        }
                        // } end
					}
    ...
    /* (non-Javadoc)
     * @see com.orangelabs.rcs.core.protocol.msrp.MsrpEventListener#msrpTransferError(java.lang.String, java.lang.String, com.orangelabs.rcs.core.protocol.msrp.MsrpSession.TypeMsrpChunk)
     */
	@Override
	...
	protected void receiveHttpFileTransfer(ContactId contact, String displayName, FileTransferHttpInfoDocument fileTransferInfo,
		...
		getImsService().getImsModule().getListener().handleFileTransferInvitation(session, isGroupChat(), contact, displayName);
...
//	public abstract void addParticipants(List<String> participants);
	...
	public void sendMsrpMessageDeliveryStatus(String fromUri, String toUri, String msgId, String status) {
		...
		String content = ChatUtils.buildCpimDeliveryReport(fromUri, toUri, imdn, getImsService().getUser().getHomeDomain());
    ...
    public void handle200OK(SipResponse resp) {
        ...
        setGeolocSupportedByRemote(resp.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_GEOLOCATION_PUSH));

        // Check if file transfer supported by remote
        setFileTransferSupportedByRemote(resp.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_FT) ||
        		resp.isFeatureTagPresent(FeatureTags.FEATURE_RCSE_FT_HTTP));
    }
