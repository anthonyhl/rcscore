...
public class OriginatingOneToOneChatSession extends OneToOneChatSession {
	...
	public OriginatingOneToOneChatSession(ImsService parent, ContactId contact, ChatMessage msg,
		...
		super(parent, contact, PhoneUtils.formatContactIdToUri(contact, parent.getUser().getHomeDomain()), msg, rcsSettings);
		// Create dialog path
			...
			if (chatMessage != null) {
				...
				if (useImdn) {
					...
					cpim = ChatUtils.buildCpimMessageWithImdn(from, to, chatMessage.getMessageId(),
							chatMessage.getContent(), chatMessage.getMimeType(), getImsService().getUser().getHomeDomain());
				} else {
