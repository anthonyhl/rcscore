...
package com.orangelabs.rcs.core.security.cert;
...
public class KeyStoreManager {
     ...
     */
    // Changed by Deutsche Telekom
    public void loadKeyStore() throws KeyStoreManagerException {
    	// Changed by Deutsche Telekom
        ...
        if ((certRoot != null) && (certRoot.length() > 0)) {
            if (!isCertificateEntry(buildCertificateAlias(certRoot))) {
            	addCertificates(certRoot);
            }
        }
        ...
        if ((certIntermediate != null) && (certIntermediate.length() > 0)) {
            if (!isCertificateEntry(buildCertificateAlias(certIntermediate))) {
                addCertificates(certIntermediate);
            }
        }
    }
     ...
     */
    private void setClientCertificateFingerprint(Certificate cert) {
	 ...
	 */
	public void updateClientCertificate(final String ipAddress, final UserProfile user) {
		try {
			if (isKeystoreExists()) {
				if (logger.isActivated()) {
				...
				Thread t = new Thread() {
					 ...
					 */
					public void run() {
						createClientCertificate(ipAddress, user.getPublicUri());
					}
				};
	...
	// Changed by Deutsche Telekom
	private synchronized void createClientCertificate(String ipAddress, String publicUri) {
		try {
...
//                    + OemCustomization.customizeString("com.orangelabs.rcs.client"));
			...
			if (logger.isActivated()) {
                logger.debug("Client certificate " + CLIENT_CERT_ALIAS + " for IP address "
                        ...
                        + getClientCertificateFingerprint() + " added");
			}
		} catch (Exception ex) {
     ...
     */
    private void createKeyStore() throws KeyStoreManagerException {
        File file = new File(getKeystorePath());
        if ((file == null) || (!file.exists())) {
            try {
...
//			}
     ...
     */
    // Changed by Deutsche Telekom
    private void addCertificates(String path) {
        try {
            // Open the existing keystore
        	// Changed by Deutsche Telekom
            KeyStore ks = loadKeyStoreFromFile();
     ...
     */
    // Changed by Deutsche Telekom
    public String getCertFingerprint(Certificate cert, String algorithm) {
    	try {
    		// Changed by Deutsche Telekom
    		if (cert != null) {
    			if (logger.isActivated()) {
    ...
    public static boolean isOwnCertificateUsed() {
    	try {
	        ...
	        if ((certRoot != null) && (certRoot.length() > 0)) {
	        	// Changed by Deutsche Telekom
	            return isCertificateEntry(buildCertificateAlias(certRoot));
	        } else {
	        	return false;
	        }
    	} catch(Exception e) {
...
//			}
     ...
     *
     */
    private void saveKeyStoreToFile(KeyStore ks){
    	if (ks == null) {
    		return;
    	}
