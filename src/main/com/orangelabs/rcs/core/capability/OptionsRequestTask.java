...
public class OptionsRequestTask implements Runnable {
	...
	private void sendOptions() {
        ...
        try {
        	...
        	dialogPath = imsModule.getSipManager().createServiceDialogPath(contactUri);     	
        	
            // Create OPTIONS request
        	if (logger.isActivated()) {
        		logger.debug("Send first OPTIONS");
        	}
	        SipRequest options = dialogPath.createOptions(featureTags);
	...
	private void handleUserNotRegistered(SipTransactionContext ctx) {
        ...
        if (RcsStatus.NO_INFO.equals(info.getRcsStatus())) {
        	...
        	// Notify listener
        	imsModule.getListener().handleCapabilitiesNotification(mContact, info.getCapabilities());
    	}
	}
	...
	private void handleUserNotFound(SipTransactionContext ctx) {
    	...
    	// Notify listener
    	imsModule.getListener().handleCapabilitiesNotification(mContact, capabilities);
	}
