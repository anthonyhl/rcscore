...
public abstract class ImsServiceSession extends Thread {
	...
	public SipManager getSipManager() {
		return getImsService().getSipManager();
	}
	...
	public void createTerminatingDialogPath(SipRequest invite) {
		dialogPath = getSipManager().createInviteDialogPath(invite);
		
		remoteDisplayName = SipUtils.getDisplayNameFromUri(invite.getFrom());
	}
    ...
    public boolean isBehindNat() {
		return getImsService().getImsModule().isBehindNat();
    }	
	...
	public void terminateSession(int reason) {
		...
		try {
        	...
        	if (dialogPath.isSigEstablished()) {
		        ...
		        getDialogPath().sendSipBye();
        	} else {
		        // Send CANCEL without waiting a response
				getDialogPath().sendSipCancel();
        	}
	...
	public void receiveBye(SipRequest bye) {
		...
		try {
			...
			getImsService().getImsModule().requestCapabilities(remote);
	...
	public void receiveCancel(SipRequest cancel) {
		...
		try {
			...
			SipResponse terminatedResp = SipUtils.createResponse(getDialogPath().getInvite(), getDialogPath()
					.getLocalTag(), 487);
			getSipManager().sendSipResponse(terminatedResp);
		} catch (Exception e) {
			if (logger.isActivated()) {
	...
	public void send180Ringing(SipRequest request, String localTag) {
    	try {
	    	SipResponse progress = SipUtils.createResponse(request, localTag, 180);
            getSipManager().sendSipResponse(progress);
    	} catch(Exception e) {
    		if (logger.isActivated()) {
	...
	public void sendErrorResponse(SipRequest request, String localTag, int code) {
		try {
	        ...
	        SipResponse resp = SipUtils.createResponse(request, localTag, code);
	...
	public void send603Decline(SipRequest request, String localTag) {
		try {
            ...
            SipResponse resp = SipUtils.createResponse(request, localTag, 603);
            getSipManager().sendSipResponse(resp);
		} catch(Exception e) {
			if (logger.isActivated()) {
	...
	public void send403Forbidden(SipRequest request, String localTag, String warning) {
		try {
			...
			SipResponse resp = request.createResponse(localTag, 403, warning);
			getSipManager().sendSipResponse(resp);
		} catch (Exception e) {
			if (logger.isActivated()) {
	...
	public void send486Busy(SipRequest request, String localTag) {
		try {
            ...
            SipResponse resp = SipUtils.createResponse(request, localTag, 486);
            getSipManager().sendSipResponse(resp);
		} catch(Exception e) {
			if (logger.isActivated()) {
	...
	public void send415Error(SipRequest request) {
		try {
	        ...
	        SipResponse resp = request.createResponse(415);
    ...
    public void sendInvite(SipRequest invite) throws SipException {
		...
		if (ctx.isSipResponse()) {
			...
			case Response.OK:
			case Response.ACCEPTED://cx add tmply for tct-stack, TODO
				// 200 OK
    ...
    public void handle407Authentication(SipResponse resp) {
        try {
            ...
            // Set the Proxy-Authorization header, @tct-stack comment: move above setInvite()
    ...
    public void handle422SessionTooSmall(SipResponse resp) {
        try {
            ...
            int minExpire = resp.getMinSessionExpirePeriod();
            if (minExpire == -1) {
                if (logger.isActivated()) {
	...
	protected String getNetworkAddress() {
		return getImsService().getImsModule().getIpAddress();
	}
}
