...
import com.orangelabs.rcs.core.ipcall.IPCallError;
...
public class UpdateSessionManager {
	...
	public SipRequest createReInvite(String[] featureTags, String content) {
		...
		try {
			...
			// create ReInvite
			reInvite = session.getDialogPath().createReInvite(featureTags, content);
			if (logger.isActivated()) {
...
//							// set received sdp response as remote sdp content
							...
							session.handleReInviteResponse(
									...
									reInviteContext);
							SipManager r = session.getImsService().getImsModule()
									.getSipManager();
    	...
    	Thread thread = new Thread() {
    			public void run(){
    		        try {  		        	   		            
    		            ...
    		            SipResponse resp = session.getDialogPath().create200OkReInviteResponse(reInvite, respFeatureTags, sdp);
