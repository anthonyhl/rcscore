...
package com.orangelabs.rcs.protocol.rtp;

import java.util.Enumeration;
import java.util.Hashtable;
import java.util.Vector;

import com.orangelabs.rcs.protocol.rtp.codec.Codec;
import com.orangelabs.rcs.protocol.rtp.format.Format;
import com.orangelabs.rcs.protocol.rtp.format.audio.AmrWbAudioFormat;
import com.orangelabs.rcs.protocol.rtp.format.audio.AudioFormat;
import com.orangelabs.rcs.protocol.rtp.format.video.H264VideoFormat;
...
public class MediaRegistry {
    ...
    public static Codec[] generateEncodingCodecChain(String encoding) {
    	if (encoding.equalsIgnoreCase(H264VideoFormat.ENCODING)) { 
	        ...
	        Codec[] chain = {
	            new com.orangelabs.rcs.protocol.rtp.codec.video.h264.JavaPacketizer()
	        };
	        return chain;
		} else { 
	...
	public static Codec[] generateDecodingCodecChain(String encoding) {
		if (encoding.equalsIgnoreCase(H264VideoFormat.ENCODING)) {
	        ...
	        Codec[] chain = {
	            new com.orangelabs.rcs.protocol.rtp.codec.video.h264.JavaDepacketizer()
	        };
	        return chain;
		} else {
			// Codec implemented in the native part
