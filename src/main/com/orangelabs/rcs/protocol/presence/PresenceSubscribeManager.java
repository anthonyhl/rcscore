...
import com.orangelabs.rcs.protocol.presence.pidf.PidfParser;
import com.orangelabs.rcs.protocol.presence.rlmi.ResourceInstance;
import com.orangelabs.rcs.protocol.presence.rlmi.RlmiDocument;
import com.orangelabs.rcs.protocol.presence.rlmi.RlmiParser;
	    	...
	    	try {
			    ...
			    if (multi.isMultipart()) {
    					...
    					try {
	    					...
	    					for(int i=0; i < list.size(); i++) {
	    						...
	    						if ((state != null) && (reason != null)) {
	    							...
	    							// Notify listener
	    					    	getImsModule().getListener().handlePresenceSharingNotification(
					...
					try {
						...
						// Notify listener
						getImsModule().getListener().handlePresenceInfoNotification(contact, presenceInfo);
			    	} catch(Exception e) {
			    		if (logger.isActivated()) {
