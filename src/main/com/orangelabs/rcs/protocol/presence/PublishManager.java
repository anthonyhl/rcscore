...
public class PublishManager extends PeriodicRefresher {
    ...
    public void periodicProcessing() {
    	...
    	try {
	        ...
	        SipRequest publish = createDialogPath().createPublish(expirePeriod, entityTag, null);
    ...
    public synchronized boolean publish(String info) {
        try {
            ...
            SipRequest publish = dialogPath.createPublish(expirePeriod, entityTag, info);
    ...
    public synchronized void unPublish() {
    	...
    	try {
	        ...
	        // Create PUBLISH request with no SDP and expire period 
	        SipRequest publish = dialogPath.createPublish(0, entityTag, null);
	...
	private void handle407Authentication(SipTransactionContext ctx) throws Exception {
    	...
    	SipRequest publish = dialogPath.createPublish(ctx.getTransaction().getRequest().getExpires().getExpires(), entityTag, dialogPath.getLocalContent());
	...
	private void handle412ConditionalRequestFailed(SipTransactionContext ctx) throws Exception {
        ...
        SipRequest publish = dialogPath.createPublish(expirePeriod, entityTag, dialogPath.getLocalContent());
	...
	private SipDialogPath createDialogPath() {
    	return imsModule.getSipManager().createServiceDialogPath(getPublicUri());
	}
