...
package com.orangelabs.rcs.protocol.presence.xdm;
...
import com.orangelabs.rcs.protocol.http.HttpAuthenticationAgent;
import com.orangelabs.rcs.protocol.http.HttpDeleteRequest;
import com.orangelabs.rcs.protocol.http.HttpGetRequest;
import com.orangelabs.rcs.protocol.http.HttpPutRequest;
import com.orangelabs.rcs.protocol.http.HttpRequest;
import com.orangelabs.rcs.protocol.http.HttpResponse;
import com.orangelabs.rcs.protocol.presence.PhotoIcon;
import com.orangelabs.rcs.protocol.presence.directory.Folder;
import com.orangelabs.rcs.protocol.presence.directory.XcapDirectoryParser;
...
public class XdmManager {
	...
	public XdmManager(ImsModule parent) {
		imsModule = parent;
		xdmServerAddr = getUser().getXdmServerAddr();
		xdmServerLogin = getUser().getXdmServerLogin();
		xdmServerPwd = getUser().getXdmServerPassword();
	}
	...
	private String getPublicUri() {
		return imsModule.getUser().getPublicUri();
	}
	...
	private HttpResponse sendHttpRequest(HttpRequest request, HttpAuthenticationAgent authenticationAgent) throws IOException, CoreException {
		...
		httpRequest += "X-3GPP-Intended-Identity: \"" + getUser().getXdmServerLogin() + "\"" + HttpUtils.CRLF;
	...
	public HttpResponse getXcapDocuments() {
		try {
			...
			String url = "/org.openmobilealliance.xcap-directory/users/" +
				HttpUtils.encodeURL(getPublicUri()) + "/directory.xml";
	...
	public HttpResponse getRcsList() {
		try {
			...
			String url = "/rls-services/users/" +
				HttpUtils.encodeURL(getPublicUri()) + "/index";
	...
	public HttpResponse getPresenceRules() {
		try {
			...
			String url = "/org.openmobilealliance.pres-rules/users/" +
				HttpUtils.encodeURL(getPublicUri()) + "/pres-rules";
	...
	public HttpResponse setPresenceRules() {
		try {
			...
			String content = new StringBuilder("<?xml version=\"1.0\" encoding=\"")
					...
					.append(getPublicUri()).append("\"/></cr:identity>")
					.append(HttpUtils.CRLF).append(" </cr:conditions>").append(HttpUtils.CRLF)
	...
	public HttpResponse addContactToGrantedList(ContactId contact) {
		try {
			...
			// URL
			String url = "/resource-lists/users/" +
					HttpUtils.encodeURL(getPublicUri()) +
					"/index/~~/resource-lists/list%5B@name=%22rcs%22%5D/entry%5B@uri=%22" +
					HttpUtils.encodeURL(PhoneUtils.formatContactIdToUri(contact, imsModule.getUser().getHomeDomain())) + "%22%5D";
