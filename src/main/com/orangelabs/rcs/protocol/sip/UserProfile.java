...
public class UserProfile {
	...
	public UserProfile(ContactId contact,
		...
		if (displayName != null) {
			publicAddress = "\"" + displayName + "\" <" + getPublicUri() + ">";		
		}
		else {
			publicAddress = getPublicUri();
		}
	}
	...
	String getDisplayName() {
		...
		if ((displayName != null) && (displayName.length() > 0)) {
			String number = PhoneUtils.extractNumberFromUri(getPublicUri());
	...
	public void setAssociatedUri(ListIterator<Header> uris) {
		...
		setPreferredUri(sipUri, telUri);
	}

	void setPreferredUri(String sipUri, String telUri) {
		if (telUri != null) {
			preferredUri = telUri;
			return ;
		}
