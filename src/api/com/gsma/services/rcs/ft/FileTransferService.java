...
public class FileTransferService extends RcsService {
     ...
     */
    public void connect() {
    	bindService(IFileTransferService.class, apiConnection);
    }
     ...
     * the contact is not supported an exception is thrown.
     * 
     * @param convId Conversation Id. {@value=null} in OTO/OTM, {@value=chatId} in group chat.
     * @param contact the remote contact Identifier
     * @param file Uri of file to transfer
     * @param attachFileIcon File icon option. If true, the stack tries to attach fileicon.
     *      Fileicon may not be attached if file is not an image or if local or remote contact does not support fileicon.
     * @return File transfer
     * @throws RcsServiceException
     */
    public FileTransfer transferFileExt(String convId, Set<ContactId> contacts, Uri file, boolean attachFileIcon) throws RcsServiceException {
        if (api != null) {
            try {
                tryToGrantAndPersistUriPermission(file);

                IFileTransfer ftIntf = api.transferFileExt(convId, new ArrayList<ContactId>(contacts), file, attachFileIcon);
                if (ftIntf != null) {
                    return new FileTransfer(ftIntf);
                } else {
                    return null;
