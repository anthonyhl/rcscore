...
public abstract class RcsService {
	...
	protected void bindService(Context ctx, Class<?> service, ServiceConnection apiConnection) {
		Intent intent = new Intent(service.getName());
		intent.setComponent(new ComponentName("com.orangelabs.rcs", 
				"com.orangelabs.rcs.service.RcsCoreService"));
		ctx.bindService(intent, apiConnection, Context.BIND_AUTO_CREATE);
	}
	
	protected void bindService(Class<?> service, ServiceConnection apiConnection) {
		bindService(ctx, service, apiConnection);
	}
}
